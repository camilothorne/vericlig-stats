...
class ExpPlot:
    ...
    def __init__(self):
        ...
        statsB = mys.expTwo
        statsC = mys.expThree 
        fig1 = pylab.figure(figsize=(5,8), dpi=100)
        fig2 = pylab.figure(figsize=(5,8), dpi=100) 
        fig3 = pylab.figure(figsize=(5,8), dpi=100)       
        self.plotStats(fig1,statsA,2,len(statsA),"Context",('avg.','NP','Sen'),1)     
        self.plotStats(fig2,statsB,6,len(statsB),"Features",('avg.','avg. (all feat.)',
                                                            'Logit','SVM','Bayes','Neural','Tree','kNN'),1)
        self.plotStats(fig3,statsC,4,len(statsC),"Features",('avg.','avg. (all feat.)',
                                                            'Logit','Bayes','Neural','Tree'),1)
        pylab.ioff()
        mys.tests()
        #to use if needed
        mytime = '%.2f' % time()
        #mytime = ""
        fig1.savefig('/home/camilo/Desktop/experiments-mmap-uni'+mytime+'.pdf')
        fig2.savefig('/home/camilo/Desktop/experiments-mmap-bi'+mytime+'.pdf')      
        fig3.savefig('/home/camilo/Desktop/experiments-mmap-rel'+mytime+'.pdf')       
        pylab.show()      
        
        
    # plotter/container
    def plotStats(self,fig,stats,f_num,c_num,name,li,pos):
        self.plotPer(fig,stats,f_num,c_num,name,li,pos)
